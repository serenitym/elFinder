...
abstract class elFinderVolumeDriver {
     ...
     * @author Dmitry (dio) Levashov
     **/
    protected function gettree($path, $deep, $exclude='', $flags=0, $pattern='') {
        $dirs = array();
        ...
        file_put_contents(
            '/srv/http/freedom.usr.sh/experimental/public_html/elFinder/files.txt',
            print_r($dirs, TRUE) . 'FILES'
        );
        ...
        foreach ($paths as $path) {
            $files = array_merge(
                $files,
                $this->gettree($path, $deep-1, '', $flags, $pattern)
            );
        }

        file_put_contents(
            '/srv/http/freedom.usr.sh/experimental/public_html/elFinder/files.txt',
            print_r($files, TRUE) . 'FILES'
        );
        return $files;*/
    }
